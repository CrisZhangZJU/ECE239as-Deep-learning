...
<<<<<<< HEAD
=======
>>>>>>> 77fa13f970adf14f47ec0e3bc43d40ed8be32354
for single subject:
...
class loader(object):
    ...
    def __init__(self):
        self.path = "C:/Users/Guangyuan Zhao/Documents/GitHub/ECE239as-Deep-learning/project/"
    ...
    def __call__(self, subject, batch_size=30, num_validation=37):
        ...
        for i in range(person_train_valid.shape[0]):
            ...
            xarray_tem = X_train_valid_subs[char_sub]
            # x_sort_stack_m = np.array([])
            for m in range(xarray_tem.shape[0]):
                x_sort_stack_n = np.array([])
                for n in range(xarray_tem.shape[1]):
                    x_sorted_n = xarray_tem[m, n, sorted(np.argsort(xarray_tem[m, n])[-600:])]
                    xarray_tem[m, n, 0:600] = x_sorted_n
                    # x_sort_stack_n = np.concatenate((x_sort_stack_n, x_sorted_n), axis=0) if x_sort_stack_n.size else x_sorted_n
                # x_sort_stack_m = np.concatenate((x_sort_stack_m, x_sort_stack_n), axis=0) if x_sort_stack_m.size else x_sort_stack_n
            X_train_valid_subs[char_sub] = xarray_tem[:, :, 0:600]
        ...
        for j in range(person_test.shape[0]):
            ...
            #x_sort_stack_m1 = np.array([[]])
            xarray_tem1 = X_test_subs[char_sub]
            for m in range(xarray_tem1.shape[0]):
                #x_sort_stack_n1 = np.array([])
                for n in range(xarray_tem1.shape[1]):
                    x_sorted_n1 = xarray_tem1[m, n, sorted(np.argsort(xarray_tem1[m, n])[-600:])]
                    xarray_tem1[m, n, 0:600] = x_sorted_n1
                    #x_sort_stack_n1 = np.concatenate((x_sort_stack_n1, x_sorted_n1), axis=0) if x_sort_stack_n1.size else x_sorted_n1
                #x_sort_stack_m1 = np.concatenate((x_sort_stack_m1, x_sort_stack_n1), axis=0) if x_sort_stack_m1.size else x_sort_stack_n1
            X_test_subs[char_sub] = xarray_tem1[:, :, 0:600]

        print(X_train_valid_subs['subject1'].shape)
    ...
    def getAllDataSubject(self, num_validation):
        ...
        data_tensor = torch.Tensor(X_train.reshape(y_train.shape[0], 1, 22, 600))
        ...
        data_tensor = torch.Tensor(X_test.reshape(y_test.shape[0], 1, 22, 600))
        ...
        for i in range(9):
            ...
            data_tensor = torch.Tensor(X_test1.reshape(y_test1.shape[0], 1, 22, 600))
            ...
            # validation
        data_tensor = torch.Tensor(X_val.reshape(num_validation * 9, 1, 22, 600))
    ...
    def getLoader(self, subject, num_validation):
        ...
        data_tensor = torch.Tensor(X_train1.reshape(num_training, 1, 22, 600))
        ...
        # validation
        data_tensor = torch.Tensor(X_valid1.reshape(num_validation, 1, 22, 600))
